...
def parse_sphinx_docopts(index):
    ...
    index = os.fspath(index)
    # if provided path does not exist, try common alternatives in same directory
    if not os.path.exists(index):
        base = os.path.dirname(index) or '.'
        candidates = [os.path.join(base, 'index.rst'),
                      os.path.join(base, 'contents.rst'),
        ...
        for c in candidates:
            if os.path.exists(c):
                found = c
                break
        # fallback: search for an rst that appears to be a gallery file
        if not found:
                ...
                for fn in files:
                    if fn.endswith('.rst'):
                        ...
                        try:
                            ...
                            continue
                        if ('sphinx-gallery' in txt) or ('.. GENERATED FROM PYTHON' in txt) or ('Gallery generated by Sphinx-Gallery' in txt):
                            found = p
                            break
                if found:
                    break
        if not found:
            logger.warning("sphinx-gallery: Documentation options could not be found; proceeding without docopts.")
            return {}
        index = found
    ...
    try:
        ...
        raise ExtensionError(f"Could not read index file '{index}': {e}")

    # Try to extract doc options; if not present, return empty opts with a warning
    if not re.search(r'\.\.\s+GENERATED FROM PYTHON|sphinx-gallery|sphinx_gallery', content, flags=re.IGNORECASE):
        logger.warning("sphinx-gallery: Documentation options could not be found in index '%s' -- proceeding without docopts.", index)
        return {}

    # (If you need more detailed parsing of docopts, add it here.)
    return {}
...
class SphinxDocLinkResolver:
    ...
    def __init__(self, config, doc_url, gallery_dir, relative=False):
        self.config = config
        # accept pathlib.Path or other path-like objects: convert to str

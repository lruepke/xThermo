...
def plot_3d(sw,phaseBoundaries,scale='linear'):
    ...
    # legend
    leg = ax.legend()
    # change legend handles to white patches with the original handle color as edge
    from matplotlib.colors import to_hex
    new_handles = []
    # support different Matplotlib versions: legend_handles (snake_case) or legendHandles (camelCase)
    ...
    for h, t in zip(handles, texts):
        # try several ways to get a representative color from the original handle
        color = None
        ...
        if color is None and hasattr(h, 'get_color'):
            color = h.get_color()
        # convert RGBA to hex when necessary
        try:

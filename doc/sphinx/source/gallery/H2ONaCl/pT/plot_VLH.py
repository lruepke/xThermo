...
def plot_3d(sw,scale='linear'):
    ...
    leg = ax.legend()
    # change legend handles to white patches with the original handle color as edge (robust across Matplotlib versions)
    from matplotlib.colors import to_hex
    handles = getattr(leg, 'legend_handles', None) or getattr(leg, 'legendHandles', None) or []
    texts = getattr(leg, 'texts', None) or (leg.get_texts() if hasattr(leg, 'get_texts') else [])
    new_handles = []
    for h, t in zip(handles, texts):
        ...
        if hasattr(h, 'get_facecolor'):
            ...
            if fc is not None and len(fc) > 0:
                color = fc[0]
        if color is None and hasattr(h, 'get_edgecolor'):
            ec = h.get_edgecolor()
            if ec is not None and len(ec) > 0:
                color = ec[0]
        if color is None and hasattr(h, 'get_color'):
            color = h.get_color()
        try:
            ...
            pass
        label = t.get_text() if hasattr(t, 'get_text') else str(t)
        new_handles.append(Patch(facecolor='white', edgecolor=color, linewidth=0.0, label=label, hatch='++++'))
    ax.legend(handles=new_handles, loc='upper left', ncol=7)
    # text
...
def benchmark_VLH(sw,mmc3='../Driesner2007b/1-s2.0-S0016703707002955-mmc3.txt'):
    ...
    fpout.write('T[C],P(Driesner)[bar],P(xThermal),P(diff)[bar],XV(Driesner)[mol],XV(xThermal)[mol],XV(diff)[mol],RhoV(Driesner)[kg/m3],RhoV(xThermal),RhoV(err),HV(Driesner)[J/kg],HV(xThermal),HV(err),XL(Driesner)[mol],XL(xThermal)[mol],XL(diff)[mol],RhoL(Driesner)[kg/m3],RhoL(xThermal),RhoL(err),HL(Driesner)[J/kg],HL(xThermal),HL(err)\n')
...
# .. tab:: IAPS84
#
#     .. csv-table:: Comparison between result of :cite:`Driesner2007Part2` and result calculated by |xThermal|.

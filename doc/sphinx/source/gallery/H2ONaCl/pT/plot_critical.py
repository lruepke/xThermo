...
# .. seealso::
#
#     The C++ member function |P_X_Critical| and python wrapper function :py:meth:`xThermal.H2ONaCl.cH2ONaCl.P_X_Critical`.
...
def benchmark_CriticalCurve(sw,mmc2='../Driesner2007b/1-s2.0-S0016703707002955-mmc2.txt'):
    ...
    fpout.write('T[deg.C],P(Driesner)[bar],P(xThermal)[bar],P(diff)[bar],X(Driesner)[mol],X(xThermal)[mol],X(diff)[mol],Rho(Driesner)[kg/m3],Rho(xThermal),Rho(err),H(Driesner)[J/kg],H(xThermal),H(err)\n')
    ...
    l,=ax.plot(T-273.15,P/1E5, color='r',label='xThermal',lw=4)
    ...
    ax.plot(T0, Rho_,lw=4,label='xThermal')
    ...
    ax.plot(T0, H_/1E6,lw=4,label='xThermal')
...
def plot_critpT_H2O(T_crit,P_crit,cmap='GnBu'):
    ...
    iaps84 = H2O.cIAPS84()
    iapws95 = H2O.cIAPWS95()  #LHR: changed from cIAPWS95_CoolProp() 

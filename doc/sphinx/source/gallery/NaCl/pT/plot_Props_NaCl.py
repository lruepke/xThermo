...
def plot_Props(salt, scale='linear',enthalpy_FV='../../../gallery/H2ONaCl/Driesner2007b/Halite_enthalpy_Vehling20220112.txt'):
    ...
    fpout.write('T[C],P[bar],H(F.V.)[J/kg],H(xThermal),H(err)\n')
    ...
    ax_cb=ax.inset_axes([0,1.02,1,0.05])
    plt.colorbar(CS,cax=ax_cb,label='Specific enthalpy (J/kg): F.V. result - xThermal',orientation='horizontal',extend='both')
    ax_cb.xaxis.set_label_position('top')
    ax_cb.xaxis.set_ticks_position('top')
...
#
#     .. csv-table:: Comparison between result of Falko Vehling(personal communication) and result calculated by |xThermal|.

...
# --------------------------
# Compare result of |xThermal| with :cite:`Driesner2007Part1`, and also compare result based on different EOS of water.
def benchmark_NaCl(salt,mmc1='../../H2ONaCl/Driesner2007a/1-s2.0-S0016703707002943-mmc1.txt'):
    ...
    if(not os.path.exists(mmc1)):
        ...
        exit()
    T0_melting, P0_melting = np.loadtxt(mmc1,skiprows=14,max_rows=int((64-14)/2),unpack=True)
    T0_sub, P0_sub = np.loadtxt(mmc1,skiprows=78,max_rows=int((400-78)/2),unpack=True)
    T0_boil, P0_boil = np.loadtxt(mmc1,skiprows=414,max_rows=int((496-414)/2),unpack=True)
    ...
    for name,T0,P0,P in zip(['Melting','Boiling','Sublimation'],[T0_melting,T0_boil,T0_sub],[P0_melting,P0_boil,P0_sub],[P_melting,P_boil,P_sub]):
        ...
        fpout.write('T[C],P(Driesner)[bar],P(xThermal),P(err)\n')
...
#     .. tab:: IAPS84
#
#         .. csv-table:: Comparison between result of :cite:`Driesner2007Part1` (|mmc1|) and result calculated by |xThermal|.

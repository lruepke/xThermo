...
def plot_prop(water, ax=None, name_fig='kappa',name_prop='Expansivity $\kappa$',unit_prop='K$^{-1}$',T=[],p=[],cmap='GnBu'):
    ...
    for i in range(0,TT.shape[0]):
        for j in range(0,TT.shape[1]):
            ...
            prop[i][j] = props.IsobaricExpansivity
    # plot
    isSaveFig=False
    ...
    if(ax==None):
        ...
        ax=plt.gca()
    CS = ax.contourf(TT-273.15,PP/1E5, np.log10(np.abs(prop)), levels=50, cmap=cmap)
    ax_cb = ax.inset_axes([1.01, 0, 0.05, 1])
    plt.colorbar(CS, cax=ax_cb, orientation='vertical',label='%s (%s) log10 scale'%(name_prop,unit_prop))
    # labels
    ax.text(0.98,0.98,water.name(),ha='right',va='top',bbox={'fc':'w','ec':'gray'}, transform=ax.transAxes)
    ax.set_xlabel('Temperature ($^{\circ}$C)')
    ax.set_ylabel('Pressure (bar)')
    if(isSaveFig): savefig('H2O_%s_%s'%(name_fig, water.name()))
    return TT-273.15,PP/1E5,prop

def plot_error(ax, XX,YY,ZZ, eosA, eosB,cmap='RdBu',unit='K$^{-1}$'):

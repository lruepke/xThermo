...
def plot_prop(water, ax=None, name_fig='kappa',name_prop='Isothermal compressibility $\kappa$',unit_prop='Pa$^{-1}$',T=[],p=[],cmap='GnBu'):
    ...
    for i in range(0,TT.shape[0]):
        for j in range(0,TT.shape[1]):
            ...
            prop[i][j] = props.IsothermalCompressibility
    # plot
    isSaveFig=False
...
def plot_error(ax, XX,YY,ZZ, eosA, eosB,cmap='RdBu',unit='Pa$^{-1}$'):

...
def plot_prop(water, ax=None,name_prop='beta',unit_prop='T$^{-1}$',H=[],p=[],cmap='GnBu'):
    ...
    if(ax==None):
        ...
        ax=plt.gca()
    CS = ax.contourf(HH/1E6,PP/1E5, np.log10(np.abs(prop)), levels=50, cmap=cmap)
...
def plot_error(ax, XX,YY,ZZ, eosA, eosB,cmap='RdBu',unit='T$^{-1}$'):

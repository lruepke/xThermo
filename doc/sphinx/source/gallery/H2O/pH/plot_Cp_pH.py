...
iaps84 = H2O.cIAPS84()
# iapws95_CoolProp = H2O.cIAPWS95_CoolProp() #LHR: coolprop not exposed to python
iapws95 = H2O.cIAPWS95()

# Calculate and plot result
def plot_prop(water, ax=None,name_prop='Cp',unit_prop='J/kg/K',H=[],p=[],cmap='GnBu'):
    ...
    if(ax==None):
        ...
        ax=plt.gca()
    CS = ax.contourf(HH/1E6,PP/1E5, np.log10(prop), levels=50, cmap=cmap)
...
def plot_error(ax, XX,YY,ZZ, eosA, eosB,cmap='RdBu',unit='J/kg/K'):

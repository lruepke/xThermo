...
*/
{
    ...
    if ((Pbars > (5000.0 + tol)) 
        ...
        // || (TdegC > (900.0 + tol)) // Zhikui: 18 Jul 2022, confirm with IAPWS95 and CoolProp, T>900 results are similar, which can be used in hydrothermal simulation
        ) {
	return 0.0;
    }
...
*/
{
	...
	*d -= dp / mPro->spro.dpd;
    ...
    // if iteration is false, also try to return results
    format_pro(t, *d, mPro, prop);
    prop->error = 1;
}

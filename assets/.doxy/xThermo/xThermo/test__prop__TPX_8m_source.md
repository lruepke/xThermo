...
# File test\_prop\_TPX.m

[**File List**](files.md) **>** [**H2ONaCl**](dir_0a7d68cdfa0215953309325142e4c674.md) **>** [**Matlab**](dir_a83d64805f95e67d3b37610e6eb407bf.md) **>** [**test\_prop\_TPX.m**](test__prop__TPX_8m.md)

[Go to the documentation of this file](test__prop__TPX_8m.md)
...
test_constX_H2ONaCl(0.1, 'IAPS84');
...
function test_constX_H2ONaCl(X0, backendName)
    ...
    T = linspace(550,650,200);
    P = linspace(800,1000,100)*1E5;
    [TT,PP] = meshgrid(T,P);
    XX=TT*0 + X0;
    ...
    subplot(rows,cols,1); contourf(TT,PP,reshape(PROP.Reg,size(TT)));colorbar; title('phase region: matlab'); 
    subplot(rows,cols,2); contourf(TT,PP,reshape(PROP.Rho,size(TT)));colorbar; title('Rho: matlab'); 
    subplot(rows,cols,3); contourf(TT,PP,reshape(PROP.h,size(TT)));colorbar; title('H: matlab'); 
    subplot(rows,cols,4); contourf(TT,PP,reshape(PROP.mu_l,size(TT)));colorbar; title('Mu_l: matlab');
    subplot(rows,cols,5); contourf(TT,PP,reshape(PROP.mu_v,size(TT)));colorbar; title('Mu_v: matlab');
    ...
    subplot(rows,cols,6); contourf(TT,PP,props.phase);colorbar; title('phase region: xThermal:PROST'); 
    subplot(rows,cols,7); contourf(TT,PP,props.Rho);colorbar; title('Rho: xThermal:PROST'); 
    subplot(rows,cols,8); contourf(TT,PP,props.H);colorbar; title('H: xThermal:PROST'); 
    subplot(rows,cols,9); contourf(TT,PP,props.Mu_l);colorbar; title('Mu_l: xThermal:PROST'); 
    subplot(rows,cols,10); contourf(TT,PP,props.Mu_v);colorbar; title('Mu_v: xThermal:PROST'); 
    
    % diff
    subplot(rows,cols,11); contourf(TT,PP,(reshape(PROP.Reg,size(TT)) - props.phase));colorbar; title('matlab - xThermal'); 
    subplot(rows,cols,12); contourf(TT,PP,(reshape(PROP.Rho,size(TT)) - props.Rho));colorbar; title('matlab - xThermal'); 
    subplot(rows,cols,13); contourf(TT,PP,(reshape(PROP.h,size(TT)) - props.H));colorbar; title('matlab - xThermal'); 
    subplot(rows,cols,14); contourf(TT,PP,(reshape(PROP.mu_l,size(TT)) - props.Mu_l));colorbar; title('matlab - xThermal'); 
    subplot(rows,cols,15); contourf(TT,PP,(reshape(PROP.mu_v,size(TT)) - props.Mu_v));colorbar; title('matlab - xThermal'); 
